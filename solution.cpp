...
void input(int head,vector<struct node*> headnodes)
{
    ...
    string name,password,algo="encrypteddatainside";
    float value;
    int id;
    ...
    struct StructValue *structvalue=new StructValue();
    string date = datetime();
    GetTime time;
    time.put(date);
    node1->Datetime=time;
    structvalue->name=encrypt(name);
    structvalue->id=encrypt(id);
    structvalue->value=encrypt(value);
    structvalue->password=encrypt(password);
    structvalue->algo=encrypt(algo);
    
    node1->structvalue1=structvalue;
    ...
    {
        FindHash[head]->child.push_back(node1);
        node1->ref=FindHash[head];
    
    }
    
    FindHash[number]=node1;
    return;
}
...
bool printverify(string password,string algo)
{
        ...
        string algo1=encrypt("encrypteddatainside");
        
        map<int,struct node*>::iterator it=FindHash.begin();
        int flag=0;
        
        for(;it!=FindHash.end(); it++)
        {
            if(it->second->structvalue1->password==password1 && it->second->structvalue1->algo==algo1)
            {
                flag=1;
                
               
                cout<<encrypt(it->second->structvalue1->name)<<endl;
                cout<<encrypt(it->second->structvalue1->id)<<endl;
                cout<<encrypt(it->second->structvalue1->value)<<endl;
...
int main()
{
    ...
    while(1)
    {
        ...
        if(num==7)
        {
            ...
            struct node* first=FindHash[nodenum1];
            struct node* newowner=FindHash[nodenum2];
        ...
        if(num==10)
        {
            ...
            struct node* first=FindHash[nodenum1];
            struct node* second=FindHash[nodenum2];
